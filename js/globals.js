...
// --- Данные финансов ---
// Дефолтные категории (используются при первом запуске и в resetAllData)
...
function cloneDefaultFinanceCategories() {
    return DEFAULT_FINANCE_CATEGORIES.map(c => ({
        ...c,
        subcategories: c.subcategories ? c.subcategories.slice() : [],
        subcategoryLimits: c.subcategoryLimits ? { ...c.subcategoryLimits } : {}
    }));
}
